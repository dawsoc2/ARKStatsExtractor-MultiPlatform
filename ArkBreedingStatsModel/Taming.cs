...
namespace ArkBreedingStatsModel
{
    public static class Taming
    {
        ...
        public static void TamingTimes(Species.Species species, int level, double tamingSpeedMultiplier, double tamingFoodRateMultiplier,
                ...
                out int neededNarcoberries, out int neededAscerbicMushrooms, out int neededNarcotics, out int neededBioToxins, out double te, out double hunger, out int bonusLevel, out bool enoughFood)
        {
            ...
            if (species != null && species.taming != null)
            {
                ...
                for (int f = 0; f < usedFood.Count; f++)
                {
                    ...
                    var food = Values.Values.V.GetTamingFood(species, foodName);
                ...
                {
                    var food = Values.Values.V.GetTamingFood(species, usedFood[i]);
                    if (food != null)
        ...
        public static int FoodAmountNeeded(Species.Species species, int level, double tamingSpeedMultiplier, string foodName, bool nonViolent = false)
        {
            if (species != null)
            {
                double affinityNeeded = species.taming.affinityNeeded0 + species.taming.affinityIncreasePL * level;
        ...
        public static int SecondsUntilWakingUp(Species.Species species, int level, double currentTorpor)
        {
            int seconds = 0;
        ...
        public static TimeSpan TamingDuration(Species.Species species, int foodQuantity, string foodName, double tamingFoodRateMultiplier, bool nonViolent = false)
        {
        ...
        public static string KnockoutInfo(Species.Species species, int level, double longneck, double crossbow, double bow, double slingshot,
                double club, double prod, double harpoon, double boneDamageAdjuster, out bool knockoutNeeded, out string koNumbers)
        {
            ...
            if (species?.taming != null)
            {
                ...
                koNumbers = (harpoon > 0 ? Math.Ceiling(totalTorpor / (306 * boneDamageAdjuster * harpoon)) + " × " + "TranqSpearBolts" + "\n" : string.Empty)
                        + (longneck > 0 ? Math.Ceiling(totalTorpor / (442 * boneDamageAdjuster * longneck)) + " × " + "ShockingTranqDarts" + "\n" : string.Empty)
                        + (longneck > 0 ? Math.Ceiling(totalTorpor / (221 * boneDamageAdjuster * longneck)) + " × " + "TranqDarts" + "\n" : string.Empty)
                        + (prod > 0 ? Math.Ceiling(totalTorpor / (226 * boneDamageAdjuster * prod)) + " × " + "ElectricProdHits" + "\n" : string.Empty)
                        + (crossbow > 0 ? Math.Ceiling(totalTorpor / (157.5 * boneDamageAdjuster * crossbow)) + " × " + "TranqArrowsCrossBow" + "\n" : string.Empty)
                        + (bow > 0 ? Math.Ceiling(totalTorpor / (90 * boneDamageAdjuster * bow)) + " × " + "TranqArrowsBow" + "\n" : string.Empty)
                        + (slingshot > 0 ? Math.Ceiling(totalTorpor / (24.5 * boneDamageAdjuster * slingshot)) + " × " + "SlingshotHits" + "\n" : string.Empty)
                        + (club > 0 ? Math.Ceiling(totalTorpor / (10 * boneDamageAdjuster * club)) + " × " + "WoodenClubHits" + "\n" : string.Empty);

                // torpor depletion per s
                string torporDepletion = string.Empty;
                if (torporDeplPS > 0)
                    torporDepletion = "\n" + "TimeUntilTorporDepleted" + ": " + Utils.Utils.DurationUntil(new TimeSpan(0, 0, (int)Math.Round(totalTorpor / torporDeplPS)))
                            + "\n" + "TorporDepletion" + ": " + Math.Round(torporDeplPS, 2)
                            + " / s;\n" + "ApproxOneNarcoberryEvery" + " " + Math.Round(7.5 / torporDeplPS + 3, 1)
                            + " s " + "OrOneAscerbicMushroom" + " " + Math.Round(25 / torporDeplPS + 3, 1)
                            + " s " + "OrOneNarcoticEvery" + " " + Math.Round(40 / torporDeplPS + 8, 1)
                            + " s " + "OrOneBioToxinEvery" + " " + Math.Round(80 / torporDeplPS + 16, 1) + " s";
        ...
        public static string QuickInfoOneFood(Species.Species species, int level, double tamingSpeedMultiplier, double tamingFoodRateMultiplier,
                string foodName, int foodAmount, string foodDisplayName)
        {
            ...
            return $"{string.Format("WithXFoodTamingTakesTime", foodAmountUsed[0], foodDisplayName, Utils.Utils.DurationUntil(duration))}\n" +
                    $"{"Narcotics"}: {narcotics}\n" +
                    $"TamingEffectiveness_Abb: {Math.Round(100 * te, 1)} %\n" +
                    $"{"BonusLevel"}: +{(level + bonusLevel)}\n" +
                    $"{string.Format("FoodHasToDropUnits", Math.Round(hunger, 1))}";
        }

        public static string BoneDamageAdjustersImmobilization(Species.Species species, out Dictionary<string, double> boneDamageAdjusters)
        {
            ...
            if (species != null)
            {
                ...
                if (species.immobilizedBy != null && species.immobilizedBy.Any())
                    text += $"{(text.Length > 0 ? "\n" : string.Empty)}{"ImmobilizedBy"}: " +
                            $"{string.Join(", ", species.immobilizedBy)}";
            }
            return text;
        }

        public static int DurationAfterFirstFeeding(Species.Species species, int level, double foodDepletion)
        {
            int s = 0;
            if (species != null &&
                species.taming != null &&

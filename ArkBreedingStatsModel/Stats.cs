...
using ArkBreedingStatsModel.Utils;
...
namespace ArkBreedingStatsModel
{
    public static class StatValueCalculation
    {
        ...
        public static double CalculateValue(Species.Species species, int stat, int levelWild, int levelDom, bool dom, double tamingEff, double imprintingBonus, bool roundToIngamePrecision = true)
        {
            if (species == null)
                return 0;
            ...
            if (dom)
            {
                ...
                if (imprintingBonus > 0
                    ...
                    imprintingM = 1 + species.StatImprintMultipliers[stat] * imprintingBonus * Values.Values.V.currentServerMultipliers.BabyImprintingStatScaleMultiplier;
                if (stat == 0)
            ...
            if (roundToIngamePrecision)
                return Math.Round(result, Utils.Utils.Precision(stat), MidpointRounding.AwayFromZero);

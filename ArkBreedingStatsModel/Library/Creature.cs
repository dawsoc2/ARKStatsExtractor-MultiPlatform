...
namespace ArkBreedingStatsModel.Library
{
    ...
    public class Creature : IEquatable<Creature>
    {
        ...
        public Creature(Species.Species species, string name, string owner = null, string tribe = null, Sex sex = Sex.Unknown, int[] levelsWild = null, int[] levelsDom = null, double tamingEff = 0, bool isBred = false, double imprinting = 0, int? levelStep = null)
        {
            Species = species;
            ...
            this.levelsDom = levelsDom ?? new int[Values.Values.STATS_COUNT];
            this.isBred = isBred;
            if (isBred)
            {
                this.tamingEff = 1;
        ...
        public Creature(long arkId)
        {
            ArkId = arkId;
            ArkIdImported = true;
            guid = Utils.Utils.ConvertArkIdToGuid(arkId);
        ...
        public void SetTopStatCount(bool[] considerStatHighlight, bool considerWastedStats)
        {
            ...
            if (topBreedingStats == null) topBreedingStats = new bool[Values.Values.STATS_COUNT];
        ...
        /// </summary>
        internal void InitializeArkInGame() => ArkIdInGame = ArkIdImported ? Utils.Utils.ConvertImportedArkIdToIngameVisualization(ArkId) : ArkId.ToString();

        private void InitializeArrays()
        {
            if (levelsDom == null) levelsDom = new int[Values.Values.STATS_COUNT];
            if (valuesBreeding == null) valuesBreeding = new double[Values.Values.STATS_COUNT];
            if (valuesDom == null) valuesDom = new double[Values.Values.STATS_COUNT];
            if (topBreedingStats == null) topBreedingStats = new bool[Values.Values.STATS_COUNT];
        }
